...
test('avisos: ajustes abre la configuración completa y conserva la prueba por dispositivo',()=>{
  const index=readFileSync(new URL('../index.html',import.meta.url),'utf8');
  const ui=readFileSync(new URL('../js/ui.js',import.meta.url),'utf8');
  assert.match(index,/id="pushSettings"/);
  assert.match(ui,/data-push="test"/);
  assert.match(ui,/notificationBefore/);
...
test('avisos: las tareas fechadas se programan y los fallos al reprogramar se muestran',()=>{
  const app=readFileSync(new URL('../js/app.js',import.meta.url),'utf8');
  assert.match(app,/const datedTask=entry\.type==="task"/);
  assert.match(app,/waitForServer:datedTask/);
  // Desde V0.26.7 la selección y el reparto en tandas viven en notification-settings.js.
  ...
  assert.match(app,/const failed=await runInBatches\(entriesToReschedule\(notes\),entry=>cloud\.schedulePush\(entry\)\);ui\.closeLayers\(\);/,"cierra siempre y avisa aparte de los fallos");
});
