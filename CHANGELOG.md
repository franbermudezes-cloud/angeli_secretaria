...
## V0.22.13 · Editar los accesos directos en dos móviles casi a la vez ya no borraba en silencio el cambio del otro

- **Causa raíz**: cada guardado de los accesos directos (`saveShortcuts` en `js/app.js`) subía a Firestore con `setDoc(shortcutsDocument(), {items, hidden})` (`js/firebase.js`) — una sobrescritura completa del documento, no una fusión. Si dos dispositivos editaban los accesos (añadir, borrar, reordenar) dentro de la misma ventana corta, el que terminaba de guardar el segundo pisaba entero el documento del primero, sin ningún error ni aviso de conflicto. Además, los accesos no tenían ningún `id` estable — se distinguían por posición o por contenido — así que ni siquiera había forma de saber si dos accesos "parecidos" en dos copias del documento eran el mismo editado o dos accesos distintos.
- **Corregido**: cada acceso directo (por defecto, preset o creado a mano) lleva ahora un `id` estable (`normalizeShortcuts` en `js/shortcuts.js` lo rellena si falta, igual que `makeId` ya hace para los artículos de la compra); los siete accesos de `DEFAULT_SHORTCUTS` llevan un id fijo en el propio código para que dos dispositivos que arrancan sin nada guardado todavía coincidan en el mismo id y no se dupliquen al sincronizar por primera vez. `saveShortcuts` (`js/firebase.js`) ya no hace un `setDoc` a ciegas: calcula qué cambió de verdad en este dispositivo desde la última sincronización (`diffShortcuts`: altas, bajas, ediciones y el orden deseado) y, dentro de una `runTransaction`, lee la copia más reciente que haya en la nube y le aplica solo esos cambios (`applyShortcutsDiff`) antes de escribir — así un acceso añadido o borrado en el otro dispositivo mientras este editaba el suyo no desaparece.
- **Alcance**: cubre altas, bajas y ediciones de accesos directos, y el ajuste de "ocultar accesos directos" (que viaja en el mismo documento). No cubre un conflicto de posición exacto entre dos reordenaciones simultáneas del mismo acceso; en ese caso gana el orden del último dispositivo en guardar, como antes.
- Sin cambios en el backend; no requiere redespliegue.
- Tests: 7 pruebas nuevas en `tests/shortcuts.test.mjs` (id estable en accesos por defecto y antiguos, `diffShortcuts` sobre altas/bajas/ediciones/orden, y dos escenarios de fusión con dos dispositivos divergiendo de la misma base sin perder ningún cambio).
