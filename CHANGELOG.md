...
## V0.22.13 · Una pregunta pendiente de otra orden ya no se comía un recordatorio, un WhatsApp, un evento o una llamada nuevos (auditoría, prioridad alta)

- **Causa raíz**: el carve-out de la lista de la compra (V previas) era, deliberadamente, la única excepción documentada a una regla más general y peligrosa: cualquier interacción a medias (`interaction.status==="awaiting_input"`) capturaba SIEMPRE la siguiente frase entera como respuesta suya, aunque fuera una orden nueva y sin relación de otro tipo. El caso más desnudo era WhatsApp: `localWhatsApp()`, al ver una pregunta pendiente por `contactName` o `notes`, daba por buena literalmente cualquier texto como respuesta a ese campo — sin comprobar si en realidad era, por ejemplo, "Recuérdame llamar al médico mañana". Y aunque ese respaldo local no aplicara, `add()` seguía pasando el contexto de la interacción activa (`noteInterpretationContext`) a la IA remota, que podía malinterpretar la frase nueva como continuación de la pregunta pendiente en vez de como una orden nueva.
- **Corregido**: se generaliza el mismo carve-out que ya protegía a la lista de la compra. Dos funciones nuevas en `js/ai.js`: `explicitNewCommandDomain(texto)` reconoce disparadores inequívocos de orden nueva — "Recuérdame..." (recordatorio), "envía/manda un whatsapp a..." (WhatsApp, reutilizando el propio `localWhatsApp` sin `active` para no duplicar su patrón), "nuevo evento"/"añade... al calendario" (evento) y "llama a..." (reutilizando `localImmediateCall`, que nunca tuvo noción de continuidad) — y `activeIntentDomain(active)` da el dominio de la interacción a medias (reminder/whatsapp/calendar/call). En `add()`, justo antes del carve-out de la compra, si el texto trae un disparador de un dominio distinto al de `active`, se descarta `active` para ese turno: se procesa como una orden completamente nueva, igual que si no hubiera nada pendiente.
- **Deliberadamente conservador**: solo se descarta `active` cuando el disparador es inequívoco y de un dominio *distinto*. Una respuesta real y corta a la pregunta pendiente ("a las nueve", "Juan Pérez", "diciendo que llego tarde", "sí, cámbialo a las nueve") no coincide con ningún disparador y sigue completando la interacción activa exactamente igual que antes.
- Sin cambios en el backend; no requiere redespliegue.
- Tests: 8 pruebas nuevas en `tests/conversation.test.mjs` — disparadores por dominio, ausencia de falsos positivos en respuestas cortas reales, el caso reportado (WhatsApp a medias no se come un recordatorio/evento/llamada nuevos, y viceversa con un recordatorio a medias y un WhatsApp nuevo), y continuaciones reales verificadas de extremo a extremo para `calendar.update`, `reminder.create` y `whatsapp.compose`.
