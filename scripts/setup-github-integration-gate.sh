...
# GitHub solo puede exigir un check cuyo workflow ya existe en la rama por
# defecto. La primera instalación se fusiona manualmente y se vuelve a ejecutar
# este script para activar la protección sin crear un bloqueo imposible.
if ! gh api "repos/${REPOSITORY}/contents/.github/workflows/integration-gate.yml?ref=main" >/dev/null 2>&1; then
  echo "BOOTSTRAP_PENDING: fusiona manualmente el primer PR y vuelve a ejecutar este script"
  gh pr view "${BRANCH}" --repo "${REPOSITORY}" --json url,state,statusCheckRollup
  exit 0
fi
