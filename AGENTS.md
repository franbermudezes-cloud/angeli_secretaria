...
- Cuando una tarea funcional tenga un alcance claro, las comprobaciones disponibles hayan pasado y no exista una decisión, riesgo o autorización externa pendiente, crear el commit y hacer `push` a `origin/main` directamente para que la PWA publicada pueda probarse. Detenerse antes de publicar solo si el usuario lo pide expresamente, si la tarea sigue incompleta o si hace falta una decisión del usuario. Los cambios documentales aislados pueden agruparse con la publicación funcional correspondiente.
...
- Si el arnés aislado aplicable está configurado, ejecutarlo antes de publicar un cambio de integración. Un fallo bloquea la publicación funcional hasta corregirlo o dejar explícitamente decidido su alcance; una prueba no automatizable se registra como manual, nunca como aprobada.
...
- Antes de publicar cambios funcionales de Calendar, Contacts, Drive, Firestore, conversación o PWA, ejecutar el arnés de pruebas reales cuando el entorno aislado esté configurado. El arnés usa exclusivamente `buengusto.es@gmail.com`, la carpeta Drive `Angeli - Pruebas` y secretos `angeli-test-google-*-grant`; jamás reutiliza datos ni autorizaciones de producción. Si el arnés no está configurado, declarar esta limitación de forma explícita y no afirmar que la integración está validada.
...
- Los adjuntos e imágenes se guardan de forma remota en Drive; no alterar sus referencias ni sus destinos configurados sin verificar sus efectos sobre las entradas existentes.
