...
- No publicar trabajo nuevo directamente en `main`. Cuando una tarea tenga alcance claro y no exista una decisión externa pendiente, crear una rama `codex/<descripcion>`, subirla y abrir un Pull Request hacia `main`. El check obligatorio `integration-gate` ejecuta automáticamente las pruebas unitarias y el arnés real aislado; si pasa, el workflow activa la fusión automática del PR. Si falla, el PR permanece abierto y se corrige en la misma rama antes de volver a intentarlo. Los cambios documentales aislados siguen este mismo flujo y pueden agruparse con la publicación funcional correspondiente.
