import { typeLabel } from "./classifier.js?v=0.21.87";
import { calendarDetails, scheduleState, scheduleTitle, scheduleWhen } from "./schedule.js?v=0.21.87";
import { noteClassificationLabel, noteTitle } from "./notes.js?v=0.21.87";
import { normalizeNoteSettings, settingLabel } from "./note-settings.js?v=0.21.87";
import { whatsappChoices, whatsappPhone } from "./whatsapp.js?v=0.21.87";
import { normalizeNotificationSettings } from "./notification-settings.js?v=0.21.87";
import { filterMediaLibrary, mediaSize } from "./media-library.js?v=0.21.87";
import { mediaContextRelation, normalizeMediaContext } from "./media-context.js?v=0.21.87";
import { groupDietarioByDay } from "./dietario.js?v=0.21.87";
...
export function createUI({ getMedia }) {
  ...
  function workingBody(detail) {
    ...
    image.src = "assets/angeli-welcome.gif?v=0.21.87";
  ...
  function showShoppingListChoice(lists, onChoose) {
    openModal({
      ...
      actions: [
        ...lists.map(list => ({ label: `${list.name} · ${list.items.length} artículo${list.items.length === 1 ? "" : "s"}`, kind: "secondary", onClick: () => onChoose(list.id) })),
        { label: "Cancelar", kind: "secondary", onClick: closeLayers }
      ]
    });
  }
