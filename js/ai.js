import{calendarQueryRange,cleanTemporalText,naturalQueryRange,temporalData}from"./temporal.js?v=0.23.1";
import{localWhatsApp}from"./whatsapp.js?v=0.23.1";
import{REMINDER_TRIGGER}from"./keywords.js?v=0.23.1";
...
function failureReason(error){const message=String(error?.message||"");if(error?.code==="quota_exhausted"||/prueba de Angeli/i.test(message))return"quota_exhausted";if(/Baja confianza/i.test(message))return"low_confidence";if(/abort/i.test(message))return"timeout";if(/\b503\b|IA no disponible/i.test(message))return"service_unavailable";return"invalid_or_unavailable"}
