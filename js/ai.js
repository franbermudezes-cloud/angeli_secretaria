import{calendarQueryRange,cleanTemporalText,temporalData}from"./temporal.js?v=0.21.32";
...
const EMPTY={title:null,date:null,time:null,rangeStart:null,rangeEnd:null,location:null,contactName:null,phone:null,notes:null,noteQuery:null,noteClassification:null,target:null,changes:null,linkedReminder:null,missingFields:[],question:null};
...
export function validateIntent(raw){if(!raw||typeof raw!=="object"||Array.isArray(raw))throw new Error("Respuesta IA no válida");const allowed=new Set(["intent","confidence","title","date","time","rangeStart","rangeEnd","location","contactName","phone","notes","noteQuery","noteClassification","target","changes","linkedReminder","requiresConfirmation","missingFields","question"]);if(Object.keys(raw).some(key=>!allowed.has(key)))throw new Error("Campo IA no permitido");if(!VALID_INTENTS.includes(raw.intent))throw new Error("Intent IA no permitido");if(typeof raw.confidence!=="number"||raw.confidence<0||raw.confidence>1)throw new Error("Confianza IA no válida");const normalized={...EMPTY,intent:raw.intent,confidence:raw.confidence,requiresConfirmation:Boolean(raw.requiresConfirmation)};for(const key of["title","location","contactName","phone","notes","noteQuery","question"]){if(raw[key]!==undefined&&raw[key]!==null){if(typeof raw[key]!=="string"||raw[key].length>MAX_TEXT_LENGTH)throw new Error("Texto IA no válido");normalized[key]=raw[key].trim()||null}}for(const key of["date","time","rangeStart","rangeEnd"]){if(raw[key]!==undefined&&raw[key]!==null){if(!isValidTemporal(key,raw[key]))throw new Error("Fecha u hora IA no válida");normalized[key]=raw[key]}}if(normalized.rangeStart&&normalized.rangeEnd&&normalized.rangeStart>=normalized.rangeEnd)throw new Error("Intervalo IA no válido");normalized.target=normalizeTarget(raw.target);normalized.changes=normalizeChanges(raw.changes);normalized.linkedReminder=normalizeLinkedReminder(raw.linkedReminder);normalized.noteClassification=normalizeNoteClassification(raw.noteClassification);normalized.missingFields=normalizeMissingFields(raw.missingFields);if(normalized.missingFields.length&&!normalized.question)normalized.question=null;if(SENSITIVE_INTENTS.has(normalized.intent))normalized.requiresConfirmation=true;return normalized}
...
function normalizeNoteClassification(value){if(value===undefined||value===null)return null;if(typeof value!=="object"||Array.isArray(value))throw new Error("Clasificación de nota no válida");const allowed=new Set(["scope","relationType","relationName","purpose","tags"]);if(Object.keys(value).some(key=>!allowed.has(key))||!["general","personal","company"].includes(value.scope)||!["none","person","client","project","event"].includes(value.relationType)||!Array.isArray(value.tags)||value.tags.length>5)throw new Error("Clasificación de nota no válida");const cleanText=key=>{const item=value[key];if(item===undefined||item===null)return null;if(typeof item!=="string"||item.length>MAX_TEXT_LENGTH)throw new Error("Clasificación de nota no válida");return item.trim()||null};const tags=[...new Set(value.tags.map(tag=>{if(typeof tag!=="string"||!tag.trim()||tag.length>60)throw new Error("Etiqueta de nota no válida");return tag.trim()}))];const relationName=value.relationType==="none"?null:cleanText("relationName");return{scope:value.scope,relationType:relationName?value.relationType:"none",relationName,purpose:cleanText("purpose"),tags}}
