import{clearNotes,deleteMediaDB,readShortcuts,writeShortcuts}from"./storage.js?v=0.21.51";
import{classify,actionData}from"./classifier.js?v=0.21.51";
import{sendEntry}from"./sheets.js?v=0.21.51";
import{createUI}from"./ui.js?v=0.21.51";
import{createGoogleIntegration}from"./google.js?v=0.21.51";
import{interpret,remoteProvider,localReminderQuery,localNoteQuery,localCalendarCancellation,localCalendarUpdate,localLinkedCalendarIntent,protectCalendarInterpretation,protectReadQuery}from"./ai.js?v=0.21.51";
import{entryTypeForIntent,planIntent}from"./intents.js?v=0.21.51";
import{calendarQueryRange,temporalData}from"./temporal.js?v=0.21.51";
import{normalizeFutureCall,normalizeReminderSchedule,normalizeUndatedCall,deferredCallIntent,scheduleFor,linkedScheduleFor,updateCalendarDetails,updateCalendarDateTime}from"./schedule.js?v=0.21.51";
import{createCloudSync}from"./firebase.js?v=0.21.51";
import{createMediaService}from"./media.js?v=0.21.51";
import{cancelInteraction,completeInteraction,contextFor,resolveConversationTurn,preserveCancellation}from"./conversation.js?v=0.21.51";
import{completionTarget,completePendingWithCalendar,findPendingMatches,findReminderMatches,markCancelledReminder}from"./pending.js?v=0.21.51";
import{createAgendaActions}from"./agenda.js?v=0.21.51";
import{prepareNoteDraft,missingNoteDraftFields,findNoteMatches,noteClassificationFromIntent,removeNoteEntry,updateNoteDraft,updateNoteStatus}from"./notes.js?v=0.21.51";
import{DEFAULT_NOTE_SETTINGS,addNoteSetting,applyExplicitNoteCategory,normalizeNoteSettings,noteInterpretationContext,removeNoteSetting,renameNoteSetting,settingLabel}from"./note-settings.js?v=0.21.51";
import{DEFAULT_SHORTCUTS,normalizeShortcuts,routeShortcutIntent,shortcutPrefix,shortcutType}from"./shortcuts.js?v=0.21.51";
import{localWhatsApp,whatsappUrl}from"./whatsapp.js?v=0.21.51";
import{DEFAULT_NOTIFICATION_SETTINGS,normalizeNotificationSettings}from"./notification-settings.js?v=0.21.51";
...
if("serviceWorker"in navigator)navigator.serviceWorker.register("sw.js?v=0.21.51",{updateViaCache:"none"}).then(registration=>registration.update()).catch(()=>{});
load();
