import{clearNotes,deleteMediaDB,readShortcuts,writeShortcuts,readShortcutsHidden,writeShortcutsHidden,readDayBriefingShown,writeDayBriefingShown}from"./storage.js?v=0.26.7";
import{classify,actionData}from"./classifier.js?v=0.26.7";
import{sendEntry}from"./sheets.js?v=0.26.7";
import{createUI}from"./ui.js?v=0.26.7";
import{createGoogleIntegration}from"./google.js?v=0.26.7";
import{interpret,remoteProvider,chatAside,fetchSpeech,protectQueryRange,searchMercadonaProduct,localReminderQuery,localNoteQuery,localCalendarCancellation,localCalendarUpdate,localLinkedCalendarIntent,localImmediateCall,activeIntentDomain,explicitNewCommandDomain,protectCalendarInterpretation,protectContactCallInterpretation,protectReadQuery}from"./ai.js?v=0.26.7";
import{parseShoppingCommand,parseItemList,addShoppingItems,removeShoppingItems,checkShoppingItems,clearShoppingList,toggleShoppingItem,removeShoppingItemById,setShoppingItemProduct,setShoppingItemQuantity,describeShoppingItems,shoppingListTotal,normalizeShoppingState,getActiveList,findListByName,createShoppingList,renameShoppingList,deleteShoppingList,setActiveShoppingList,setShoppingListStore,isMercadonaList,updateListItems,addCheckedToCart,toggleCartItem,setCartItemQuantity,removeCartItem,finalizePurchase}from"./shopping.js?v=0.26.7";
import{entryTypeForIntent,planIntent}from"./intents.js?v=0.26.7";
import{calendarQueryRange,temporalData}from"./temporal.js?v=0.26.7";
import{normalizeFutureCall,normalizeReminderSchedule,normalizeUndatedCall,deferredCallIntent,scheduleFor,linkedScheduleFor,updateCalendarDetails,updateCalendarDateTime,calendarDetails}from"./schedule.js?v=0.26.7";
import{createCloudSync}from"./firebase.js?v=0.26.7";
import{createMediaService}from"./media.js?v=0.26.7";
import{cancelInteraction,completeFromCollected,completeInteraction,confirmationReply,contextFor,isCancelReply,isNewCommand,resolveConversationTurn,preserveCancellation}from"./conversation.js?v=0.26.7";
import{completionTarget,completePendingWithCalendar,findPendingMatches,findReminderMatches,markCancelledReminder}from"./pending.js?v=0.26.7";
import{createAgendaActions,dayBriefing}from"./agenda.js?v=0.26.7";
import{prepareNoteDraft,missingNoteDraftFields,findNoteMatches,noteClassificationFromIntent,removeNoteEntry,updateNoteDraft,updateNoteStatus}from"./notes.js?v=0.26.7";
import{DEFAULT_NOTE_SETTINGS,addNoteSetting,applyExplicitNoteCategory,normalizeNoteSettings,noteInterpretationContext,removeNoteSetting,renameNoteSetting,settingLabel}from"./note-settings.js?v=0.26.7";
import{DEFAULT_SHORTCUTS,SHORTCUT_PRESETS,normalizeShortcuts,routeShortcutIntent,shortcutPrefix,shortcutType}from"./shortcuts.js?v=0.26.7";
import{localWhatsApp,whatsappUrl}from"./whatsapp.js?v=0.26.7";
import{speechText}from"./speech.js?v=0.26.7";
import{DEFAULT_NOTIFICATION_SETTINGS,normalizeNotificationSettings,entriesToReschedule,runInBatches}from"./notification-settings.js?v=0.26.7";
import{mediaLibraryItems}from"./media-library.js?v=0.26.7";
import{mediaContextComplete,normalizeMediaContext}from"./media-context.js?v=0.26.7";
...
function prepareShortcut(shortcut){pendingShortcut=shortcut;if(shortcut.command){$("text").value=shortcut.command;autosize();add({shortcut});return}const prefix=shortcutPrefix(shortcut);$("text").value=prefix;$("text").placeholder=shortcut.prompt||"Escribe o dicta tu instrucción…";autosize();openDraft();/* 3ª auditoría: desde V0.23.2 el borrador no saca el teclado solo, y los accesos sin dictado (Recordatorio, Nuevo evento, Nota…) se abrían sin teclado NI micro. Voz primero: todos empiezan escuchando; el teclado sigue a un toque (⌨️). */setTimeout(start,120);ui.notify(shortcut.prompt||"Te escucho")}
...
function openDraft(focus=false){ui.showDraft({value:$("text").value,focus,onInput:value=>{if(dictationMic.isActive()){if(rec)rec.discarded=true;stop()}$("text").value=value;finalText=value;autosize()},onSend:add,onMic:()=>start({inConversation:true}),onCancel:()=>{stopStrayDictation();pendingShortcut=null;ui.closeLayers()}})}
 ...
 ui.showNoteConfirmation(entry,{
  ...
  onSave:async()=>{
   ...
   try{
    ...
    // PRIVACIDAD: la hoja de Google Sheets es el registro del propietario; lo de
    // una persona invitada nunca se copia ahí.
    if(cloud.session().owner){try{await sendEntry(entry,new Date());ui.notify("Nota registrada en Google")}catch(_){ui.notify("Nota sincronizada · Google Sheets no respondió")}}
...
// Pedido por el propietario: fuera del modo conversación, si la orden se ha
// DICTADO con el micrófono, Angeli contesta con una frase corta («¡Apuntado!»,
// «¿Y a qué hora?») sin quedarse escuchando. Si se ha escrito, sigue callada.
let composerDictated=false;
async function add(options={}){
 const speak=composerDictated&&!conversationOn&&voicePrefs.speakReplies!==false;
 composerDictated=false;
 await addEntry(options);
 if(speak)await speakBriefOutcome();
}
async function speakBriefOutcome(){
 const kind=conversationModalKind();
 const title=$("modalTitle").textContent||"",lead=$("modalLead").textContent||"";
 let text="";
 if(kind==="completion")text=ui.spokenModalText()||[title,lead].filter(Boolean).join(". ");
 else if(kind==="question")text=lead||title;
 else if(kind==="manual")text=ui.spokenModalText()||(title==="Tu agenda"?lead:title);
 else if($("toast").classList.contains("show"))text=$("toast").textContent;
 if(text)await speakAloud(text);
}
async function addEntry({interactionId=null,shortcut=null}={}){
 if(saving)return;
 ...
 // 3ª auditoría: el acceso «🛒 Añadir a la compra» (sin acción propia) se
 // saltaba este paso y acababa como nota. Solo se lo salta un acceso con acción.
 if(!shortcutContext?.action&&!pendingImages.length&&!pendingFiles.length){
 ...
 if(active?.interaction?.status==="pending_confirmation"){
   ...
   if(reply==="no"){await saveConfirmed(notes.map(item=>item.id===active.id?cancelInteraction(item):item));clearComposer();ui.closeLayers();ui.notify("Vale, lo dejo");return}
   ui.showEntryAction(active,google);return;
 }
 const now=new Date(),images=[],files=[],hasMedia=Boolean(pendingImages.length||pendingFiles.length);let id=active?.id||crypto.randomUUID(),mediaUploaded=false;if(rec)rec.discarded=true;stopStrayDictation();saving=true;setSending(true);ui.showWorking("Procesando tu instrucción","Angeli ha recibido tu petición.","Un momento, no pulses Enviar otra vez.");try{
   ...
   const forceWhatsApp=Boolean(whatsApp)&&(aiWhatsApp||interpreted.source!=="ai"||Boolean(localWhatsApp(text,null))||(active?.aiIntent?.intent==="whatsapp.compose"&&(active.interaction?.missingFields||[]).includes("notes")));
   ...
   const normalizedInterpretation=protectQueryRange(normalizeReminderSchedule(normalizeFutureCall(normalizeUndatedCall(withCollected,text,active,now),text),text,now),text,now);
   ...
   clearComposer();
   if(!active&&cloud.session().owner){try{await sendEntry(entry,now);ui.notify("Entrada registrada en Google")}catch(e){ui.notify("Entrada sincronizada · Google Sheets no respondió")}}
   if(entry.interaction.status==="awaiting_input")continueConversation(entry);
   else {
     // Reportado por el propietario: preguntó «¿qué tengo pendiente el mes de
     // octubre?», la app lo escribió y no contestó nada. La consulta solo se
     // lanzaba sola desde un acceso directo; dicha a mano esperaba a que se
     // tocara «📅 Consultar». Consultar la agenda no cambia nada, así que se
     // busca siempre y se contesta directamente (como ya se hace con contactos).
     if(interpretation.intent==="calendar.delete"||interpretation.intent==="calendar.update"||interpretation.intent==="calendar.query"){
     ...
     // Antes de proponer un evento nuevo, se mira si esa hora ya está ocupada.
     if(entry.proposal?.intent==="calendar.create"&&entry.scheduledDate&&entry.scheduledTime){
       ui.updateWorking("Mirando tu agenda","Angeli está comprobando si esa hora está libre…","");
       await google.checkCalendarClash(entry);
     }
...
function clearComposer(){if(rec)rec.discarded=true;stopStrayDictation();$("text").value="";$("text").placeholder="Escribe o dicta tu instrucción…";autosize();finalText="";pendingShortcut=null;clearPendingMedia()}
...
async function resolveReminderQuery(interpretation,{onAdd}={}){
 clearComposer();
 try{
 ...
 const reopen=()=>resolveReminderQuery(interpretation,{onAdd});
 ...
 ui.showReminderResults(matches,interpretation.target?.title||"",{onSelect:showDetail,onAdd})
}
async function cancelActive(entry){await saveConfirmed(notes.map(item=>item.id===entry.id?cancelInteraction(item):item));ui.closeLayers();ui.notify("Vale, lo dejo")}
...
function start({inConversation=false,draftId=null}={}){const SR=window.SpeechRecognition||window.webkitSpeechRecognition;if(dictationMic.isActive()){stop();return}stopSpeaking();unlockAngeliAudio();
 ...
 const target=draftId?$(draftId):$("text");finalText=target?.value.trim()||"";const isMobileDictation=/Android|iPhone|iPad|iPod/i.test(navigator.userAgent)||(/Macintosh/.test(navigator.userAgent)&&navigator.maxTouchPoints>1);rec=new SR();const recognizer=rec;rec.lang="es-ES";rec.continuous=!isMobileDictation;rec.interimResults=true;rec.maxAlternatives=1;const dictationBase=finalText||"";let committed="";// Real detectado: dictar en el buscador de la lista de la compra ("café")
...
// 3ª auditoría (regresiones de V0.23.1/0.23.3, halladas ejecutando el start()
// real contra un reconocedor simulado):
//   · En el móvil cada enunciado se suma UNA vez, al cerrar su sesión (onend):
//     si el motor entregaba el mismo final dos veces, se duplicaba («hola hola»).
//   · Los reinicios tienen límite: sin conexión era un bucle sin fin y en
//     silencio pitaba cada pocos segundos para siempre. Tras ~8 sesiones seguidas
//     sin oír nada (≈1 min) se para; un error de red en el móvil también para.
//   · Un onend tardío de un reconocedor ya parado no toca el estado compartido
//     si ya hay otro dictado en marcha (apagaba el icono y rompía el nuevo).
//   · Tras Enviar (clearComposer), el onend del reconocedor descartado ya no
//     vuelve a escribir en el cuadro el texto recién enviado.
let sessionPart="",idleSessions=0;
rec.onresult=e=>{idleSessions=0;composerDictated=true;let live="";if(isMobileDictation){let finalPart="";for(let i=0;i<e.results.length;i++){const r=e.results[i],phrase=(r[0]?.transcript||"").trim();if(!phrase)continue;if(r.isFinal)finalPart=phrase;else live=phrase}sessionPart=finalPart||live}else{let finals="";for(let i=0;i<e.results.length;i++){const r=e.results[i],phrase=(r[0]?.transcript||"").trim();if(!phrase)continue;if(r.isFinal)finals+=(finals?" ":"")+phrase;else live+=(live?" ":"")+phrase}committed=finals}const shown=isMobileDictation?[committed,sessionPart].filter(Boolean).join(" "):committed;finalText=((dictationBase?dictationBase+" ":"")+(isMobileDictation?committed:shown)).trim();paint(((dictationBase?dictationBase+" ":"")+shown+(!isMobileDictation&&live?(shown?" ":"")+live:"")).trim())};rec.onerror=e=>{const fatal=e.error==="not-allowed"||e.error==="service-not-allowed"||e.error==="audio-capture"||(isMobileDictation&&e.error==="network");if(fatal){dictationMic.set(false);setMicState(false);$("hint").textContent="Dictado detenido.";ui.notify(e.error==="not-allowed"?"Permiso de micrófono denegado":e.error==="network"?"Sin conexión: el dictado necesita internet":"No se pudo usar el micrófono");return}if(!isMobileDictation&&e.error!=="no-speech"&&e.error!=="aborted")ui.notify("Error de dictado: "+e.error)};rec.onend=()=>{if(rec&&rec!==recognizer)return;if(recognizer.discarded)return;if(isMobileDictation){if(sessionPart){committed=(committed?committed+" ":"")+sessionPart;sessionPart=""}else idleSessions+=1}finalText=((dictationBase?dictationBase+" ":"")+committed).trim();paint(finalText);if(isMobileDictation&&dictationMic.isActive()&&rec===recognizer&&idleSessions<8){try{recognizer.start();return}catch(_){/* si no reinicia, terminamos abajo */}}if(rec===recognizer)rec=null;dictationMic.set(false);setMicState(false);$("hint").textContent=idleSessions>=8?"Dictado parado tras un rato en silencio. Toca el micro para seguir.":"Dictado terminado. Puedes continuar o pulsar Enviar cuando acabes.";if(shortcutCapture&&finalText){shortcutCapture=false;createShortcut(finalText);$("text").value="";finalText=""}if(!draftId)autosize()};try{rec.start()}catch(e){dictationMic.set(false);setMicState(false);ui.notify("No se pudo iniciar el dictado")}}
...
// Botón «confirmar» del modal abierto que espera un «sí/no» dicho en voz alta.
let conversationConfirm=null;
const VOICE_CONFIRM_ACTIONS=new Set(["calendar","calendar-bundle","schedule"]);
...
// «angeli» (por defecto) es la voz propia, Vindemiatrix, generada en el servidor.
const ANGELI_VOICE="angeli";
function useAngeliVoice(){return(voicePrefs.voiceURI||ANGELI_VOICE)===ANGELI_VOICE}
function selectedVoice(){
 const voices=availableVoices();
 if(!voices.length)return null;
 const byUri=voicePrefs.voiceURI&&voicePrefs.voiceURI!==ANGELI_VOICE&&voices.find(v=>v.voiceURI===voicePrefs.voiceURI);
...
function renderVoiceOptions(){
 ...
 own.value=ANGELI_VOICE;own.textContent="Angeli (voz propia · necesita conexión)";
 select.prepend(own);
 select.value=useAngeliVoice()?ANGELI_VOICE:(current?.voiceURI||ANGELI_VOICE);
...
// Habla con la voz propia de Angeli si está elegida (y hay conexión); si el
// servidor no responde o el navegador no deja reproducir, usa la voz del
// teléfono. Cada llamada nueva corta la anterior, como antes.
let speechTurn=0;
const angeliAudio=typeof Audio==="function"?new Audio():null;
const speechBlobs=new Map();
const SILENT_MP3="data:audio/mpeg;base64,";
// iOS solo deja reproducir audio tras un toque: se «desbloquea» el reproductor
// con un silencio en el mismo toque que abre el modo conversación o la prueba.
function unlockAngeliAudio(){if(!angeliAudio)return;try{angeliAudio.src=SILENT_MP3;void angeliAudio.play().catch(()=>{})}catch(e){}}
function stopSpeaking(){speechTurn++;try{window.speechSynthesis?.cancel()}catch(e){}try{angeliAudio?.pause()}catch(e){}}
async function speakAloud(text){
 stopSpeaking();
 const turn=speechTurn,spoken=speechText(text);
 if(!spoken)return;
 if(useAngeliVoice()&&angeliAudio){
  try{
   const rate=voicePrefs.rate||1,key=`${rate}|${spoken}`;
   let blob=speechBlobs.get(key);
   if(!blob){blob=await fetchSpeech(spoken,rate,await cloud.getAuthToken());if(spoken.length<=80){speechBlobs.set(key,blob);if(speechBlobs.size>40)speechBlobs.delete(speechBlobs.keys().next().value)}}
   if(turn!==speechTurn)return;
   await playAngeliAudio(blob,turn);
   return;
  }catch(e){if(turn!==speechTurn)return}
 }
 await speakWithDevice(spoken);
}
function playAngeliAudio(blob,turn){
 return new Promise((resolve,reject)=>{
  const url=URL.createObjectURL(blob);let done=false;
  const finish=()=>{if(done)return;done=true;angeliAudio.onended=angeliAudio.onpause=angeliAudio.onerror=null;URL.revokeObjectURL(url);resolve()};
  angeliAudio.onended=finish;angeliAudio.onpause=finish;angeliAudio.onerror=finish;
  angeliAudio.src=url;
  angeliAudio.play().catch(error=>{if(done)return;done=true;URL.revokeObjectURL(url);turn===speechTurn?reject(error):resolve()});
 });
}
...
async function conversationHandleOutcome(){
 ...
 // Respuesta directa a una pregunta de agenda: se dice tal cual y se sigue
 // escuchando (la lista queda en pantalla), sin pedir que se toque nada.
 if(kind==="manual"&&$("modalTitle").textContent==="Tu agenda"){
  const spoken=$("modalLead").textContent;
  ui.addConversationTurn("angeli",spoken);
  ui.setConversationStatus("Angeli está hablando…");
  await speakAloud(spoken);
  ui.setConversationStatus("Te escucho");
  watchForModalClose(()=>{if(conversationOn)resumeConversationListening()});
  resumeConversationListening();
  return;
 }
 if(kind==="manual"){
  ...
  // Si el modal trae una frase natural para decir, se usa esa en vez de leer
  // el título y la explicación como un formulario.
  const natural=ui.spokenModalText();
  // Pedido por el propietario: en el modo conversación, un «sí» dicho en voz
  ...
  await speakAloud(spoken);
  watchForModalClose(()=>{conversationConfirm=null;if(conversationOn)resumeConversationListening()});
  if(confirmable&&confirmable.isConnected){conversationConfirm=confirmable;resumeConversationListening()}
  return;
 }
 const toast=$("toast");
...
// «Sí» / «no» a la confirmación que Angeli acaba de preguntar en voz alta.
// «Sí» pulsa el mismo botón que se tocaría (misma acción, mismas
// comprobaciones); «no» cierra sin hacer nada. Cualquier otra frase sigue su
// camino normal como orden o corrección.
async function conversationAnswerConfirm(text){
 const button=conversationConfirm;
 conversationConfirm=null;
 const reply=button&&button.isConnected&&$("actionModal").classList.contains("show")?confirmationReply(text):null;
 if(!reply)return false;
 conversationBusy=true;
 ui.addConversationTurn("me",text);
 conversationModalObserver?.disconnect();conversationModalObserver=null;
 if(reply==="no"){ui.closeLayers();ui.notify("Vale, lo dejo")}
 else{ui.setConversationStatus("Angeli está en ello…");button.click();await waitForConfirmResult(button)}
 conversationBusy=false;
 await conversationHandleOutcome();
 return true;
}

// Espera a que la acción del botón termine: el botón desaparece y ya no se
// muestra el «trabajando…» (máximo 30 s).
async function waitForConfirmResult(button){
 const started=Date.now();
 while(Date.now()-started<30000){
  const modal=$("actionModal");
  if(!button.isConnected&&!(modal.classList.contains("show")&&modal.classList.contains("working-modal")))return;
...
function startConversationRecognizer(){
 ...
 conversationRec.onend=()=>{
  ...
  if(!manualStop&&!conversationTurnDispatched&&conversationOn&&!conversationBusy&&(conversationModalKind()!=="manual"||conversationConfirm))startConversationRecognizer();
 };
...
// PRIVACIDAD: Calendar, Contactos y Drive usan todavía la autorización del
// propietario; una persona invitada no las ve (el servidor además las bloquea).
function refreshOwnerUI(){const session=cloud.session(),owner=Boolean(session.owner),guest=session.signedIn&&!owner;$("adminSection").hidden=!owner;const note=$("guestIntegrationsNote");if(note)note.hidden=!guest;if(!owner)stopAdmin()}
...
$("remindersOpen").onclick=()=>void resolveReminderQuery(localReminderQuery("Recordatorios pendientes"),{onAdd:newReminderDraft});
// Pedido por el propietario: desde las pantallas de arriba solo se podía mirar.
// Cada una tiene ahora su «＋» para añadir ahí mismo (como ya tenía el Dietario).
function newReminderDraft(){ui.closeLayers();prepareShortcut({label:"Recordatorio",prompt:"¿Qué quieres que te recuerde y cuándo?",prefix:"Recuérdame ",action:"reminder.create"})}
function newNoteDraft(){prepareShortcut({label:"Nota",prompt:"Escribe o dicta la nota.",prefix:"",action:"note"})}
function addFromLibrary(){
 const pick=input=>{ui.closeMediaLibrary();$(input).click()};
 if(libraryState.kind==="image")return pick("photoInput");
 if(libraryState.kind==="file")return pick("fileInput");
 ui.openModal({title:"¿Qué quieres añadir?",lead:"Se guardará en tu Drive y aparecerá aquí.",actions:[{label:"🖼️ Foto",kind:"secondary",onClick:()=>{ui.closeLayers();pick("photoInput")}},{label:"📎 Archivo",kind:"secondary",onClick:()=>{ui.closeLayers();pick("fileInput")}},{label:"Cancelar",kind:"secondary",onClick:ui.closeLayers}]});
}
$("libraryAdd").onclick=addFromLibrary;
$("noteLibraryAdd").onclick=()=>{ui.closeNoteLibrary();newNoteDraft()};
...
$("voiceReplies").checked=voicePrefs.speakReplies!==false;
$("voiceReplies").onchange=()=>{voicePrefs.speakReplies=$("voiceReplies").checked;saveVoicePrefs()};
$("voiceTest").onclick=()=>{unlockAngeliAudio();void speakAloud("¡Hola! Soy Angeli. Así sonaré a partir de ahora.")};
...
if("serviceWorker"in navigator)navigator.serviceWorker.register("sw.js?v=0.26.7",{updateViaCache:"none"}).then(registration=>registration.update()).catch(()=>{});
load();
