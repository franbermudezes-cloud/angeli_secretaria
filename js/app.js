import{clearNotes,deleteMediaDB,readShortcuts,writeShortcuts,readShortcutsHidden,writeShortcutsHidden}from"./storage.js?v=0.22.24";
import{classify,actionData}from"./classifier.js?v=0.22.24";
import{sendEntry}from"./sheets.js?v=0.22.24";
import{createUI}from"./ui.js?v=0.22.24";
import{createGoogleIntegration}from"./google.js?v=0.22.24";
import{interpret,remoteProvider,chatAside,searchMercadonaProduct,localReminderQuery,localNoteQuery,localCalendarCancellation,localCalendarUpdate,localLinkedCalendarIntent,localImmediateCall,activeIntentDomain,explicitNewCommandDomain,protectCalendarInterpretation,protectContactCallInterpretation,protectReadQuery}from"./ai.js?v=0.22.24";
import{parseShoppingCommand,parseItemList,addShoppingItems,removeShoppingItems,checkShoppingItems,clearShoppingList,toggleShoppingItem,removeShoppingItemById,setShoppingItemProduct,setShoppingItemQuantity,describeShoppingItems,shoppingListTotal,normalizeShoppingState,getActiveList,findListByName,createShoppingList,renameShoppingList,deleteShoppingList,setActiveShoppingList,setShoppingListStore,isMercadonaList,updateListItems,addCheckedToCart,toggleCartItem,setCartItemQuantity,removeCartItem,finalizePurchase}from"./shopping.js?v=0.22.24";
import{entryTypeForIntent,planIntent}from"./intents.js?v=0.22.24";
import{calendarQueryRange,temporalData}from"./temporal.js?v=0.22.24";
import{normalizeFutureCall,normalizeReminderSchedule,normalizeUndatedCall,deferredCallIntent,scheduleFor,linkedScheduleFor,updateCalendarDetails,updateCalendarDateTime,calendarDetails}from"./schedule.js?v=0.22.24";
import{createCloudSync}from"./firebase.js?v=0.22.24";
import{createMediaService}from"./media.js?v=0.22.24";
import{cancelInteraction,completeInteraction,contextFor,resolveConversationTurn,preserveCancellation}from"./conversation.js?v=0.22.24";
import{completionTarget,completePendingWithCalendar,findPendingMatches,findReminderMatches,markCancelledReminder}from"./pending.js?v=0.22.24";
import{createAgendaActions}from"./agenda.js?v=0.22.24";
import{prepareNoteDraft,missingNoteDraftFields,findNoteMatches,noteClassificationFromIntent,removeNoteEntry,updateNoteDraft,updateNoteStatus}from"./notes.js?v=0.22.24";
import{DEFAULT_NOTE_SETTINGS,addNoteSetting,applyExplicitNoteCategory,normalizeNoteSettings,noteInterpretationContext,removeNoteSetting,renameNoteSetting,settingLabel}from"./note-settings.js?v=0.22.24";
import{DEFAULT_SHORTCUTS,SHORTCUT_PRESETS,normalizeShortcuts,routeShortcutIntent,shortcutPrefix,shortcutType}from"./shortcuts.js?v=0.22.24";
import{localWhatsApp,whatsappUrl}from"./whatsapp.js?v=0.22.24";
import{DEFAULT_NOTIFICATION_SETTINGS,normalizeNotificationSettings}from"./notification-settings.js?v=0.22.24";
import{mediaLibraryItems}from"./media-library.js?v=0.22.24";
import{mediaContextComplete,normalizeMediaContext}from"./media-context.js?v=0.22.24";
...
function createShoppingListPrompt(){
 ui.showShoppingNamePrompt({title:"Nueva lista",lead:"Ponle un nombre — por ejemplo, Fran, Mamá o Casa.",placeholder:"Nombre de la lista",confirmLabel:"Siguiente",onSave:name=>{
  ui.showShoppingStoreChoice(null,async store=>{
...
if("serviceWorker"in navigator)navigator.serviceWorker.register("sw.js?v=0.22.24",{updateViaCache:"none"}).then(registration=>registration.update()).catch(()=>{});
load();
