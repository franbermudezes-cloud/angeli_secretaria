...
## 2026-09-20 — Editar los accesos directos en dos móviles casi a la vez borraba en silencio el cambio del otro V0.22.13

**Causa raíz**: `saveShortcuts()` (`js/app.js`) mutaba el array de accesos en local y llamaba a `cloud.saveShortcuts(shortcuts, shortcutsHidden)`, que en `js/firebase.js` hacía `setDoc(shortcutsDocument(), {items, hidden})` — una sobrescritura completa del documento, sin fusión de ningún tipo. `onShortcuts` (el `onSnapshot` de ese documento, en `js/app.js`) también reemplazaba entero el array local en cuanto llegaba cualquier actualización remota. Si dos dispositivos editaban los accesos (añadir, quitar, reordenar) dentro de la misma ventana corta, el `setDoc` que llegaba en segundo lugar pisaba sin avisar el cambio del primero — sin error ni aviso de conflicto. La raíz de fondo era que los accesos no tenían ningún `id` estable (`js/shortcuts.js`, `DEFAULT_SHORTCUTS`/`SHORTCUT_PRESETS` eran objetos `{label,command,action,...}` sueltos, comparados por posición o por contenido) — sin id no hay forma de distinguir "este acceso se editó" de "es un acceso distinto que coincide por casualidad", que es lo mínimo que hace falta para fusionar en vez de sobrescribir.

**Corrección**: cada acceso directo lleva ahora un `id` estable. `normalizeShortcuts` lo rellena si falta (mismo patrón que `makeId` en `js/shopping.js`, pero llamado `makeShortcutId`); los siete `DEFAULT_SHORTCUTS` llevan un id fijo en el propio código fuente (`default-hoy`, `default-llamar-contacto`, etc.) para que dos dispositivos que arrancan sin nada guardado en Firestore todavía (y por tanto caen los dos en `DEFAULT_SHORTCUTS` como base) terminen de acuerdo en qué id le corresponde a cada uno — si el id se generara al azar en cada llamada, la primera sincronización los habría duplicado. `saveShortcuts` en `js/firebase.js` cambia de firma (`saveShortcuts(previous, items, hidden)`: recibe también la copia local de antes de la edición) y ya no hace un `setDoc` a ciegas: `diffShortcuts` (`js/shortcuts.js`) calcula qué cambió de verdad en este dispositivo desde la última sincronización (altas, bajas, ediciones y el orden deseado por id), y dentro de una `runTransaction` se lee la copia más reciente que haya en Firestore y se le aplica solo ese diff (`applyShortcutsDiff`) antes de escribir — así un acceso añadido o borrado en el otro dispositivo mientras este editaba el suyo sigue ahí después de guardar. `app.js` mantiene `shortcutsBaseline` (la última copia sincronizada) para poder calcular ese diff en cada guardado, y la actualiza con el resultado ya fusionado que devuelve la transacción.

**Cobertura de test**: `tests/shortcuts.test.mjs` ampliado — comprueba que los accesos por defecto y los reconstruidos desde uno antiguo sin id reciben un id estable y reproducible, que `diffShortcuts` detecta correctamente altas/bajas/ediciones/orden, y dos escenarios de fusión con dos "móviles" divergiendo de la misma base (uno borra mientras el otro ya había subido un acceso nuevo; los dos añaden un acceso distinto a la vez) donde ningún cambio se pierde. También se actualizaron las aserciones existentes sobre la firma antigua de `cloud.saveShortcuts`.
