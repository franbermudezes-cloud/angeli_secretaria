...
**Causa raíz**: cuando se añadió el carrito (V0.22.11), el check de la lista cambió de significado — de "ya comprado" a "lo quiero esta vez" (es el paso previo a "🛒 Añadir al carrito", que copia lo marcado al carrito y lo deja sin marcar en la lista). Pero la limpieza del cambio de significado se quedó a medias: la interfaz de la lista seguía llamando "comprado" a lo marcado en TODOS los sitios (cabecera de sección `<div class="day">Comprado</div>`, subtítulo "N comprados", tarjeta de lista en el resumen, aria-label del check "Marcar como comprado"), y el botón "Quitar comprados" (`shoppingClearChecked`, `js/app.js`) borraba los artículos marcados de forma permanente SIN ninguna confirmación — a diferencia de "Vaciar lista", que sí confirma desde V0.22.20. Escenario real: marcas leche y pan queriendo pasarlos al carrito, los ves agrupados bajo "Comprado", tocas "Quitar comprados" pensando que limpia lo ya comprado, y los pierdes de la lista sin aviso.

**Corrección**: (1) `shoppingClearChecked` ahora abre un modal de confirmación nuevo, `showShoppingRemoveMarkedConfirm(count,...)` (`js/ui.js`), que además recuerda que para pasar al carrito hay que usar "Añadir al carrito"; si no hay nada marcado, avisa en vez de no hacer nada. (2) Relabel completo de "comprado"→"marcado" en la interfaz de la lista: botón "Quitar marcados" (`index.html`), cabecera "Marcados" y subtítulo "N marcados" (`renderShoppingDetail`), subtítulo de la tarjeta de lista en el resumen (`shoppingListCardMarkup`), aria-label del check ("Marcar (lo quiero esta vez)"/"Desmarcar"), y el aviso del comando de voz ("Marcado: ..." en vez de "Marcado como comprado: ..."). No se toca la lógica pura de `shopping.js` (clearShoppingList con `onlyChecked:true` sigue igual) — solo el etiquetado y la confirmación previa.

**Fuera de alcance**: el carrito y el historial sí usan "comprado" con su sentido correcto (finalizar una compra archiva lo realmente comprado) — eso no se toca; el relabel es solo en la LISTA, donde marcar significa "lo quiero", no "lo compré".

**Cobertura de test**: `tests/shopping.test.mjs` — nueva prueba que comprueba que el handler pide confirmación (`showShoppingRemoveMarkedConfirm`), que sigue borrando solo lo marcado (`onlyChecked:true`), que el modal existe y está exportado, y que la interfaz ya no dice "comprado" (botón "Quitar marcados", cabecera "Marcados", aria sin "comprado"). Verificado en vivo: cabecera "Marcados", subtítulo "2 marcados", botón "Quitar marcados", y el modal "¿Quitar los marcados?" antes de borrar.
...
**Causa raíz**: `showDietarioDetail` (`js/ui.js`) mostraba el detalle de un evento/aviso/evento+aviso combinado ya confirmado con una única acción: `{label:"Cerrar",kind:"confirm",onClick:closeLayers}`. La misma entrada, si se encontraba por voz ("¿qué recordatorios tengo?", vía `resolveReminderQuery`), sí tenía una ficha completa con "Modificar"/"Cancelar aviso" (`showReminderDetail`/`showReminderEditor`/`showReminderCancellation`, ya construidas para ese flujo) — pero el Dietario, que es la vía más directa para repasar lo ya guardado, no las reutilizaba. Para eventos sueltos ni siquiera existía ese flujo alternativo: no había ninguna vía para editar o anular un evento de Calendar ya creado sin pasar por buscarlo de nuevo desde una consulta.

**Corrección**: `showDietarioDetail(note,{onEdit,onCancelEvent,onCancelSchedule})` (`js/ui.js`) gana un tercer parámetro con las nuevas acciones — condicionadas a que la parte correspondiente esté de verdad activa (`eventEditable` exige `calendarStatus==="synced"`; `reminderEditable` exige `schedule?.status==="scheduled"`), para no ofrecer "editar" algo que todavía está pendiente de crearse o que ya se canceló. `openDietarioEntry` (`js/app.js`) pasa tres funciones nuevas:
- `openDietarioCalendarMenu(note)`: reutiliza el mismo patrón de menú de campos ya usado en `showEntryAction` antes de confirmar (título/fecha y hora/ubicación/descripción, y "Cambiar aviso" si va combinado con un recordatorio), pero cada campo lleva a `openDietarioCalendarFieldEditor`/`openDietarioCalendarDateTimeEditor`, que reutilizan los editores YA EXISTENTES (`showCalendarFieldEditor`/`showCalendarDateTimeEditor`) sin cambiarlos, y al guardar aplican `updateCalendarDetails`/`updateCalendarDateTime` (ya usadas para la edición previa a confirmar) sobre la entrada, y ADEMÁS sincronizan con Calendar de verdad — antes de confirmar no hacía falta, porque el evento no existía todavía en Calendar; aquí sí.
- `cancelDietarioEvent`/`cancelDietarioReminder`: piden confirmación y llaman a las funciones de Google correspondientes.

**Nuevo en `js/google.js`**: `updateSyncedCalendarEntry(note)` — PATCHea directamente por el id ya guardado en la propia entrada (`calendarEventId`/`schedule.calendarEventId`), sin depender de ningún resultado de búsqueda en caché (a diferencia de `updateCalendarEvent`/`updateListedCalendarEvent`, que sí dependen de `calendarResults`). Sirve para los tres casos con el mismo código: un evento suelto (solo actualiza el evento), un recordatorio suelto (solo actualiza el aviso, reutilizando el mismo payload que ya construye `updateScheduledReminder`) y un evento+aviso combinados (actualiza los dos, ya que `updateCalendarDateTime` desplaza ambas fechas proporcionalmente cuando van juntos). `cancelSyncedCalendarEvent(note)` — cancela solo la parte de evento (a diferencia de `cancelScheduledReminder`, que ya existía para el aviso), marcando `calendarStatus:"cancelled"`, el mismo estado que `calendarActions()` en `ui.js` ya sabe mostrar como "✓ Evento cancelado".

**Verificado en el navegador sandbox**: un evento sincronizado muestra "✎ Modificar"/"Anular evento" (y tocar "Anular evento" dispara la acción correcta); un recordatorio programado muestra "Cancelar aviso"; un evento todavía PENDIENTE de crear en Calendar (`calendarStatus:"pending"`) sigue mostrando solo "Cerrar", como antes — evitando ofrecer editar/anular algo que Calendar ni siquiera conoce todavía.

**Cobertura de test**: `tests/dietario.test.mjs` ampliado — comprueba la nueva firma de `showDietarioDetail` y sus condiciones `eventEditable`/`reminderEditable`, que `openDietarioEntry` pasa las tres funciones nuevas, que cada una existe y llama a la función de Google correcta, y que `updateSyncedCalendarEntry`/`cancelSyncedCalendarEvent` existen y se exportan desde `createGoogleIntegration`.

## 2026-09-20 — Una pregunta pendiente de otra orden se comía un recordatorio, WhatsApp, evento o llamada nuevos V0.22.15

Noveno hallazgo de prioridad media de la auditoría completa que se corrige (implementado por una sesión en segundo plano, revisado e integrado igual que el resto de esta ronda). Ya estaba documentado como un hallazgo conocido y deliberadamente parcheado solo para la lista de la compra (ver la entrada de V0.22.1 para el contexto de la auditoría) — la propia auditoría reconfirmó que el mismo fallo seguía siendo alcanzable para recordatorios, calendario y WhatsApp.

**Causa raíz**: `conversationActiveQuestionEntry()` (`js/app.js`) coge la primera nota con `interaction.status==="awaiting_input"`, sin mirar de qué intención es esa pregunta pendiente, y `add({interactionId:active.id})` la trata como el destino de cualquier frase nueva. Para la lista de la compra ya existía un carve-out explícito (comentario "La lista de la compra es una función nueva y separada de las notas..." en `add()`) que reconoce un comando inequívoco ANTES de tocar el intérprete y lo procesa como algo nuevo, sin mirar `active`. Ese mismo tratamiento no existía para el resto de dominios: un WhatsApp a medias esperando el nombre del contacto o el texto del mensaje (`localWhatsApp` con `missingFields` `contactName`/`notes`) daba por buena CUALQUIER texto como respuesta, sin comprobar si en realidad era una orden nueva y sin relación — y aunque ese respaldo local no aplicase, el contexto de `active` se le pasaba igualmente a la IA remota (`noteInterpretationContext`), que podía malinterpretar la continuidad.

**Corrección**: dos funciones puras nuevas en `js/ai.js`. `explicitNewCommandDomain(texto)` detecta, de forma determinista y sin tocar el intérprete, si el texto trae un disparador inequívoco de orden nueva de un dominio concreto — reutiliza `localWhatsApp(valor,null)` y `localImmediateCall` (que ya son disparadores explícitos, sin necesitar contexto de conversación) y añade disparadores propios para recordatorio ("recuérdame.../recuerda.../acuérdate...") y para crear evento ("nuevo evento"/"añade... al calendario"). `activeIntentDomain(active)` traduce la intención de la interacción a medias a ese mismo vocabulario de dominios (reminder/whatsapp/calendar/call). En `add()` (`js/app.js`), justo después de resolver `active`, si su dominio difiere del que detecta `explicitNewCommandDomain(text)`, `active` se pone a `null` para ese turno — se procesa como si no hubiera ninguna pregunta pendiente, igual que ya hacía el carve-out de la compra, en vez de arriesgarse a que el pipeline (local o remoto) lo confunda con una continuación.

**Deliberadamente conservador**: `explicitNewCommandDomain` solo reconoce disparadores inequívocos, no cualquier frase nueva — una respuesta real y corta a la pregunta pendiente ("a las nueve", "Juan Pérez", "sí, cámbialo a las nueve", "diciendo que llego tarde") no dispara ningún dominio y sigue completando la interacción activa exactamente igual que antes.

**Cobertura de test**: `tests/conversation.test.mjs` ampliado con 8 pruebas — disparadores por dominio, ausencia de falsos positivos en respuestas cortas reales, `activeIntentDomain` por intención, el escenario reportado en ambas direcciones (un WhatsApp a medias no se come un recordatorio/evento/llamada nuevos, y viceversa con un recordatorio a medias y un WhatsApp nuevo), y tres continuaciones reales de extremo a extremo (`calendar.update`, `reminder.create`, `whatsapp.compose`) que confirman que una respuesta genuina sigue resolviéndose como antes.

## 2026-09-20 — Sincronización de accesos directos entre dos dispositivos a la vez V0.22.14

Octavo hallazgo de prioridad media de la auditoría completa que se corrige (implementado por una sesión en segundo plano, lanzada tras la propia auditoría; revisado e integrado igual que el resto de esta ronda).

**Causa raíz**: `saveShortcuts(items, hidden)` (`js/firebase.js`) hacía `setDoc(shortcutsDocument(), {items, hidden})` — una sobrescritura completa del documento en Firestore, sin fusión ninguna. Dos móviles con Angeli abierto a la vez, cada uno tocando sus accesos directos (uno crea uno nuevo, el otro borra otro), producían dos `setDoc` casi simultáneos: el que llegaba en segundo lugar sobrescribía el documento entero con SU copia local, que no incluía el cambio que el otro dispositivo acababa de guardar — el acceso añadido o borrado en el otro móvil desaparecía sin ningún aviso. La raíz de fondo era más profunda que "falta una transacción": los accesos directos no llevaban ningún `id` propio (se identificaban por su contenido/posición), así que no había forma de saber si dos objetos "parecidos" en dos copias distintas eran el mismo acceso editado o dos accesos distintos que coincidían por casualidad — sin eso, ninguna fusión automática podía ser fiable.

**Corrección**: `DEFAULT_SHORTCUTS` (`js/shortcuts.js`) gana un `id` fijo por cada acceso base (`default-hoy`, `default-recordatorio`...) para que dos dispositivos que arrancan sin nada guardado coincidan en los mismos ids; `normalizeShortcuts` genera un id estable (`sc-<timestamp36>-<random>`) para cualquier acceso que todavía no tuviera uno, y lo conserva en normalizaciones sucesivas. Con esos ids, `diffShortcuts(before,after)` calcula qué cambió de verdad en este dispositivo desde la última sincronización (altas, bajas, ediciones y el orden deseado) y `applyShortcutsDiff(remote,diff)` aplica exactamente esos cambios sobre la copia remota — conservando lo que el remoto tenga y este dispositivo no conocía. `saveShortcuts` en `js/firebase.js` ahora recibe también `previous` (la copia local tal y como estaba antes de la edición) y, dentro de una `runTransaction`, lee la copia más reciente de Firestore, le aplica el diff calculado y escribe el resultado — devolviéndolo para que `js/app.js` actualice su copia local con lo que de verdad quedó guardado (que puede incluir cambios del otro dispositivo). `app.js` mantiene ahora `shortcutsBaseline` (la última copia sincronizada) junto a `shortcuts` (la copia de trabajo local) para poder calcular ese diff en cada guardado (`saveShortcuts()`, `toggleShortcutsHidden()`) y actualizarla con el resultado fusionado.

**Cobertura de test**: `tests/shortcuts.test.mjs` ampliado con 4 pruebas — ids estables en `DEFAULT_SHORTCUTS` y tras `normalizeShortcuts` (incluida la migración de un acceso antiguo sin id), `diffShortcuts` detectando altas/bajas/ediciones/orden, y dos escenarios de fusión real (un borrado local no debe perder un alta remota que este dispositivo no conocía; dos altas simultáneas en dispositivos distintos deben coexistir) — 178 pruebas en total, todas en verde.
