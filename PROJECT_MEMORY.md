...
## 2026-09-20 — Una pregunta pendiente de otra orden se comía un recordatorio, un WhatsApp, un evento o una llamada nuevos V0.22.13

Reconfirmación de la auditoría completa del código: el mismo fallo de raíz que ya se había corregido de forma NARROW para la lista de la compra (ver la entrada de V0.22.1 para el contexto original de esa auditoría, y el comentario en `js/app.js` sobre `add()`, líneas ~229-247, "La lista de la compra es una función nueva...") seguía reproducible para el resto de intenciones sensibles: recordatorios, WhatsApp, calendario y llamadas.

**Causa raíz**: `conversationRunTurn` (`js/app.js`) fuerza siempre cualquier frase nueva de una utterance a apuntar a la interacción activa vía `add({interactionId:active.id})` cuando `conversationActiveQuestionEntry()` encuentra CUALQUIER nota con `interaction.status==="awaiting_input"`, sin mirar a qué intención pertenece esa pregunta. Dentro de `add()`, esto se traducía en dos vías de contaminación distintas:
1. **Los respaldos locales "continuando"**: `localWhatsApp(text,active)` (`js/whatsapp.js`), al ver `active.aiIntent.intent==="whatsapp.compose"` con la interacción esperando `contactName` o `notes`, devolvía `{...prior,[field]:value||null,missingFields:[],question:null}` para CUALQUIER `value` — sin ninguna comprobación de que el texto realmente fuera una respuesta plausible a esa pregunta y no una orden nueva y sin relación. Es la versión más desnuda del fallo: no hacía falta que la IA remota se equivocara, el respaldo local ya secuestraba el texto por su cuenta. (`localCalendarUpdate`, en cambio, ya tenía una comprobación de contenido más estricta — solo acepta cambios con verbo de calendario, fecha/hora o vocabulario de evento — que limitaba bastante este mismo problema para `calendar.update`, aunque no lo eliminaba del todo).
2. **El contexto pasado a la IA remota**: aunque ningún respaldo local aplicase, `add()` siempre construye `noteInterpretationContext(contextFor(active),noteSettings)` y se lo pasa a `interpret()` — la IA remota recibe la intención, los campos pendientes y la pregunta de la interacción activa como contexto de conversación, y puede (razonablemente, dado ese contexto) interpretar una frase nueva y sin relación como una respuesta a esa pregunta en vez de como una orden nueva.

**Corrección**: se generaliza el carve-out de la compra en vez de tocar cada respaldo local uno a uno (que hubiera significado repetir la misma comprobación de dominio en `localWhatsApp`, `localCalendarUpdate`, y en el punto donde se decide invocar a la IA). Dos funciones nuevas y puras en `js/ai.js`:
- `explicitNewCommandDomain(texto, ahora)`: reconoce disparadores inequívocos de una orden nueva y a qué dominio pertenece — `"reminder"` para "Recuérdame..."/"Recuerda..."/"Acuérdate..." (mismo patrón ya usado, y ya validado como inequívoco, en `localReminderQuery`/`localNoteQuery`/`localImmediateCall`); `"whatsapp"` reutilizando literalmente `localWhatsApp(texto,null)` — pasar `active=null` a propósito para que solo compruebe si el texto POR SÍ SOLO dispara una orden nueva de WhatsApp ("envía/manda un whatsapp a..."), sin su rama de continuación; `"calendar"` para "nuevo evento"/"añade... al calendario"; `"call"` reutilizando `localImmediateCall(texto,ahora)`, que nunca tuvo noción de `active` y ya excluye internamente fechas/horas explícitas y la superposición con "recuérdame llamar a...".
- `activeIntentDomain(active)`: da el dominio de la interacción a medias a partir de `active.aiIntent.intent` (`reminder.create`→`reminder`, `whatsapp.compose`→`whatsapp`, `calendar.create`/`calendar.update`/`calendar.delete`→`calendar`, `contact.call`→`call`).

En `add()` (`js/app.js`), justo después de resolver `active` y antes del carve-out de la compra: si `active.interaction.status==="awaiting_input"` y `explicitNewCommandDomain(text)` da un dominio distinto de `activeIntentDomain(active)`, se pone `active=null` para ese turno — exactamente el mismo efecto que ya tenía el carve-out de la compra (bypasear `active` del todo), solo que generalizado a cualquier dominio en conflicto en vez de solo a la compra. Con `active=null`, el resto del pipeline (local y remoto) procesa el texto como una orden completamente nueva y sin contexto contaminado, tal y como ya funcionaba para "añade colacao a la lista de la compra" desde V0.1x.

**Deliberadamente conservador**: la comprobación exige un disparador léxico inequívoco Y que el dominio detectado difiera del de `active` — no basta con que el texto "no encaje" con la pregunta pendiente. Una respuesta real y corta a la interacción activa ("a las nueve", "Juan Pérez", "diciendo que llego tarde", "sí, cámbialo a las nueve") no coincide con ningún disparador de `explicitNewCommandDomain` y sigue completando `active` exactamente igual que antes de este cambio — verificado explícitamente con `resolveConversationTurn` de extremo a extremo para `calendar.update`, `reminder.create` y `whatsapp.compose`. Queda como límite conocido y aceptado (mismo tipo de trade-off que ya asume el resto de respaldos deterministas del proyecto): si el CONTENIDO real de un mensaje de WhatsApp que se está dictando empezara literalmente por "recuérdame..." se leería como una orden nueva de recordatorio en vez de como el texto del mensaje — un caso extremadamente raro en el uso real, y no distinto en espíritu de otras ambigüedades ya aceptadas en `localCalendarUpdate`/`localWhatsApp`.

**Cobertura de test**: `tests/conversation.test.mjs` ampliado con 8 pruebas — disparadores reconocidos por dominio (`explicitNewCommandDomain`), ausencia de falsos positivos en respuestas cortas reales a una pregunta pendiente, mapeo de dominios (`activeIntentDomain`), el escenario (a) reportado en ambas direcciones (un WhatsApp a medias no se come un recordatorio/evento/llamada nuevos, y un recordatorio a medias no se come un WhatsApp nuevo) con verificación cruzada de que `app.js` de verdad invoca esta comprobación antes de llamar al intérprete, y el escenario (b): tres continuaciones reales de extremo a extremo (`calendar.update`, `reminder.create`, `whatsapp.compose`) que comprueban que `resolveConversationTurn` sigue completando la interacción activa y llega a `pending_confirmation` con los datos correctos.
